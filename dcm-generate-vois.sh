...
BASE_DIR="/mnt/praxic/pdnetworks2"
NL="
"   # new line
SEP="---------------------------------------------------------------------"
...
if [ -e ${VOI_FILE} ]; then
	...
	if [ -e /mnt/praxic/pdnetworks2/subjects/${subj}/session1/mcvsa_results/SPM.mat ]; then
		...
		echo "matlabbatch{1}.spm.util.voi.spmmat = cellstr(fullfile('${BASE_DIR}', '/subjects/', '${subj}', '/session1/', '${results_dir}', '/SPM.mat'));"
	    ...
	    echo "SPM.mat does not exist: $subj" >&2
        fi
    done
else
