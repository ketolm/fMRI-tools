    ...
    for line in f.readlines():
        ...
        if len(line) > 1:
            ...
            if isDefinitionString(line):
                ...
                # command.
                raise Exception("Uninterpretable command: {0}".format(line))
...
that can be used in an SPM script.
    ...
    print("% " + "-" * 66 +" %\n")
    print("clear DCM;")
    print("load(fullfile('/mnt/praxic/pdnetworks2/subjects', '%s', 'session1', '%s', 'SPM.mat'));" %
        (p, f))

    # Loads the VOIs
    print("\n% --- The VOIs " + '-' * 53 + " %")
    for i in range(len(model.vois)):
        print("load(fullfile('/mnt/praxic/pdnetworks2/subjects', '%s', 'session1', '%s', 'VOI_%s_1.mat'), 'xY');" %
              (p, f, model.vois[i]))
        print("DCM.xY(%d) = xY;\n" % (i+1))
    ...
    print("save(fullfile('/mnt/praxic/pdnetworks2/subjects', '%s', 'session1', '%s', '/DCM_results', 'DCM_%s.mat'));" %
          (p, f, model.name))
    print("disp('Estimating model %s for subject %s');" % (model.name, p))
    print("spm_dcm_estimate(fullfile('/mnt/praxic/pdnetworks2/subjects', '%s', 'session1', '%s', '/DCM_results', 'DCM_%s.mat'));" %
          (p, f, model.name))
