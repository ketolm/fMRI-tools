...
for condTYPE in ${CONDITIONS[@]}; do
    ...
    ${FSLDIR}/bin/fslsplit ${condPATH}/${condTYPE}_e002.nii.gz ${condPATH}/e002_on_;\
    ${FSLDIR}/bin/fslsplit ${condPATH}/${condTYPE}_e003.nii.gz ${condPATH}/e003_on_;\

    # apply tranformation calculated by mcflirt to each 3D volume for echo 2/3
    for ii in `ls -f ${condPATH}/e002_on_*.nii.gz`; do
	echo ${ii};\
	MM=`echo "${ii}" | awk -F'[_.]' '{print $3}'`;\
	echo ${MM[@]};\
	${FSLDIR}/bin/flirt -in ${ii} -ref ${condPATH}/refvol_e001.nii.gz -applyxfm -init ${condPATH}/${condTYPE}_e001_mc.mat/MAT_${MM} -out ${condPATH}/e002_on_mc_${MM}.nii.gz;\
    done
    echo "Made it to e003"
    for ii in `ls -f ${condPATH}/e003_on_*.nii.gz`; do
	echo ${ii};\
	MM=`echo "${ii}" | awk -F'[_.]' '{print $3}'`;\
	echo ${MM[@]};\
	${FSLDIR}/bin/flirt -in ${ii} -ref ${condPATH}/refvol_e001.nii.gz -applyxfm -init ${condPATH}/${condTYPE}_e001_mc.mat/MAT_${MM} -out ${condPATH}/e003_on_mc_${MM}.nii.gz;\
    done

    #merge the files back to 1 4D volumes for echo 2/3
    ${FSLDIR}/bin/fslmerge -t ${condPATH}/${condTYPE}_e002_mc.nii.gz ${condPATH}/e002_on_mc_*.nii.gz;\
    ${FSLDIR}/bin/fslmerge -t ${condPATH}/${condTYPE}_e003_mc.nii.gz ${condPATH}/e003_on_mc_*.nii.gz;\
    rm -rf ${condPATH}/e002_on_*.nii.gz ${condPATH}/e003_on_*.nii.gz
