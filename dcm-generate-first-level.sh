...
# ------------------------------------------------------------------
#
# MODIFIED MY MICAH TO FIT HIS PROJECT FALL 2019
#
# ------------------------------------------------------------------
# Usage
# -----
...
HLP_MSG="
 ...
 is ignored).
   ...
   * TR: the repetition time.  By default, TR = 2.5 NOW
...
MOTION_REGRESSORS=0
FUNC_FOLDER=mcvsa
STRUCT_FOLDER=WELLTHISISFUCKEDUP
...
base=/mnt/praxic/pdnetworks2/
J=1

for subj in "$@" ; do
    echo "Generating model for $subj" >&2
    [ -d $subj ] || continue
    cd ${base}/subjects/${subj}/${FUNC_FOLDER}
    ...
    echo "matlabbatch{$J}.spm.stats.fmri_spec.dir = {'${base}/subjects/${subj}/session1/${results_dir}/'}"
	...
	for ((image=1; image<=N; ++image)); do 
	    echo "'${base}/${subj}/session1/${FUNC_FOLDER}/${session},${image}'"
	done
    done

    echo "};"
    echo "matlabbatch{$J}.spm.stats.fmri_spec.sess($S).cond = struct('name', {}, 'onset', {}, 'duration', {}, 'tmod', {}, 'pmod', {});"
    echo "matlabbatch{$J}.spm.stats.fmri_spec.sess($S).multi = {'${base}/subjects/${subj}/session1/mcvsa/SPMDesignMatrix.mat'};"
    echo "matlabbatch{$J}.spm.stats.fmri_spec.sess($S).regress = struct('name', {}, 'val', {});"

    # for k in sessions
    K=1
    NS=`ls sws*.nii | wc | awk '{print $1}'`
    NS=$((NS - 1))
