...
# History
# -------
#  2019-11-05 : * Micah fucked this one up to his own needs
#
...
## ---------------------------------------------------------------- ##
J=0
DIR=/mnt/praxic/pdnetworks2
#M="${DIR}/group-analysis.m"
#echo "%% Starting... " > ${M}

L1_RESULTS_FOLDER=$1  # Subjects results folder
L2_RESULTS_FOLDER=$2  # Group results folder
...
# if no /all folder in L2_RESULTS_FOLDER, create it
if [ ! -d ${DIR}/${L2_RESULTS_FOLDER}/all ]; then
    echo "Warning: Creating 'all' folder" >&2
    mkdir /mnt/praxic/pdnetworks2/group_results/all
    cd ${DIR}
fi
...
while read contrast; do
	...
	echo "Working with subject $subject ... " >&2
	if [ ! -e ${DIR}/subjects/${subject_folder}/session1/do-not-include.txt ]; then
	    if [ -e ${DIR}/subjects/${subject_folder}/session1/${L1_RESULTS_FOLDER}/${contrast_file}.img ]; then
		echo "'${DIR}/subjects/${subject_folder}/session1/${L1_RESULTS_FOLDER}/${contrast_file}.img,1'"
	    elif [ -e ${DIR}/subjects/${subject_folder}/session1/${L1_RESULTS_FOLDER}/${contrast_file}.nii ]; then
		echo "'${DIR}/subjects/${subject_folder}/session1/${L1_RESULTS_FOLDER}/${contrast_file}.nii,1'"
		echo "sUCCESS" >2&
	    else
		echo "No contrast file ${DIR}/subjects/${subject_folder}/session1/${L1_RESULTS_FOLDER}/${contrast_file} for ${subject_folder}: Subject excluded" >&2
	    fi
	else
...
if [ $N == 2 ]; then
	...
	if [ ! -d ${DIR}/${L2_RESULTS_FOLDER}/${group} ]; then
	    ...
	    mkdir ${DIR}/${L2_RESULTS_FOLDER}/${group}
	fi
	C=0
	while read contrast; do
            # Generates a different subfolder for each contrast
		...
		echo "Warning: Creating '${contrast_dir}' folder" >&2
		mkdir ${DIR}/${L2_RESULTS_FOLDER}/${group}/${contrast_dir}
	    fi
		...
		#echo "   $subject"
		if [ ! -e ${DIR}/subjects/${subject}/session1/do-not-include.txt ]; then
		    if [ -e ${DIR}/subjects/${subject}/session1/${L1_RESULTS_FOLDER}/${contrast_file}.img ]; then
			# This handles Analyze format / SPM5-8
			echo "'${DIR}/subjects/${subject}/session1/${L1_RESULTS_FOLDER}/${contrast_file}.img,1'"
			
		    elif [ -e ${DIR}/subjects/${subject}/session1/${L1_RESULTS_FOLDER}/${contrast_file}.nii ]; then
			# This handles Nifti format / SPM12
			echo "'${DIR}/subjects/${subject}/session1/${L1_RESULTS_FOLDER}/${contrast_file}.nii,1'"
...
if [ $N == 2 ]; then
	...
	mkdir ${DIR}/${L2_RESULTS_FOLDER}/comparisons
    fi
	...
	if [ ! -d ${DIR}/${L2_RESULTS_FOLDER}/comparisons/${contrast_dir} ]; then
	    echo "Warning: Creating '${contrast_dir}' folder" >&2
	    mkdir ${DIR}/${L2_RESULTS_FOLDER}/comparisons/${contrast_dir}
	fi
